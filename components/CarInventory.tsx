            ...
            <section id="courses" className="py-16">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-12">
                        <h2 className="text-2xl md:text-4xl font-bold mb-4 text-parsa-gray-800">زبان مورد نظر خود را انتخاب کنید</h2>
        ...
        <section id="courses" className="py-16 bg-white">
            <div className="container mx-auto px-4">
                <div className="text-center mb-8">
                    <h2 className="text-2xl md:text-4xl font-bold mb-4 text-parsa-gray-800">دوره های زبان {selectedLanguage}</h2>
                ...
                <div className="mb-8 p-4 bg-parsa-gray-50 rounded-lg flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-2">
                             <button onClick={() => handleFilterChange('type', 'all')} className={`px-3 py-1 text-sm rounded-full ${filters.type === 'all' ? 'bg-parsa-blue-500 text-white' : 'bg-white text-parsa-gray-600'}`}>همه</button>
                             <button onClick={() => handleFilterChange('type', 'گروهی')} className={`px-3 py-1 text-sm rounded-full ${filters.type === 'گروهی' ? 'bg-parsa-blue-500 text-white' : 'bg-white text-parsa-gray-600'}`}>گروهی</button>
                             <button onClick={() => handleFilterChange('type', 'خصوصی')} className={`px-3 py-1 text-sm rounded-full ${filters.type === 'خصوصی' ? 'bg-parsa-blue-500 text-white' : 'bg-white text-parsa-gray-600'}`}>خصوصی</button>
                        </div>
                    </div>
                     <div className="flex items-center gap-2">
                        <span className="font-medium text-sm text-parsa-gray-700">فرمت کلاس:</span>
                        <div className="flex gap-2">
                             <button onClick={() => handleFilterChange('format', 'all')} className={`px-3 py-1 text-sm rounded-full ${filters.format === 'all' ? 'bg-parsa-blue-500 text-white' : 'bg-white text-parsa-gray-600'}`}>همه</button>
                             <button onClick={() => handleFilterChange('format', 'حضوری')} className={`px-3 py-1 text-sm rounded-full ${filters.format === 'حضوری' ? 'bg-parsa-blue-500 text-white' : 'bg-white text-parsa-gray-600'}`}>حضوری</button>
                             <button onClick={() => handleFilterChange('format', 'آنلاین')} className={`px-3 py-1 text-sm rounded-full ${filters.format === 'آنلاین' ? 'bg-parsa-blue-500 text-white' : 'bg-white text-parsa-gray-600'}`}>آنلاین</button>
                        </div>
                    </div>
                </div>

                <div className="text-center mb-8">
                     <button onClick={handleGoBack} className="text-parsa-blue-600 hover:text-parsa-blue-800 hover:underline inline-flex items-center gap-2 transition-colors font-medium">
