...
const LanguageCard: React.FC<LanguageCardProps> = ({ language, onSelectLanguage }) => {
    const encodedName = encodeURIComponent(language.name.replace(/\s/g, '-'));
    
    // SEO FIX: Using <a> tag instead of div onClick ensures crawlers can find the specific language pages.
    ...
    return (
        <a 
            href={`#/language/${encodedName}`}
            onClick={handleClick}
            title={`دوره های آموزش زبان ${language.name} در شیراز`}
            // Common styles for both mobile and desktop wrappers
            className="group bg-white rounded-xl shadow-lg transition-all duration-300 ease-in-out hover:shadow-2xl animate-fade-in cursor-pointer overflow-hidden block"
        >
            ...
            <div className="flex sm:hidden items-center p-3 gap-3 relative flex-row-reverse"> {/* flex-row-reverse for RTL layout */}
                <img
                    src={language.image}
                    alt={`آموزش زبان ${language.name} در شیراز`}
